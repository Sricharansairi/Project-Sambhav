    ...
    <div className="flex flex-col h-[400px] bg-background/50 border border-white/10 rounded-lg overflow-hidden relative">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.map((msg, i) => (
          ...
          >
            <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 ${msg.role === 'user' ? 'bg-[#ffb7c5]/20 text-[#ffb7c5]' : 'bg-white/10 text-muted-foreground'}`}>
              {msg.role === 'user' ? <User className="w-3.5 h-3.5" /> : <Bot className="w-3.5 h-3.5" />}
            </div>
            <div className={`text-xs leading-relaxed max-w-[85%] p-3 rounded-lg ${msg.role === 'user' ? 'bg-[#ffb7c5]/10 border border-primary/20 text-[#ffb7c5]-foreground' : 'bg-white/5 border border-white/10 text-muted-foreground'}`}>
              {msg.content}
            </div>
          </motion.div>
        ))}
      ...
      <div className="p-3 border-t border-white/10 bg-background/80">
        <div className="relative">
          ...
          <button 
            ...
            className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 text-muted-foreground hover:text-[#ffb7c5] transition-colors disabled:opacity-50"
          >
