          ...
          {messages.map((msg, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
              ...
              <div className={`flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center
                ${msg.role === 'user' ? 'bg-[#00fff2]/20 border border-[#00fff2]/30' : 'bg-white/10 border border-white/15'}`}>
                {msg.role === 'user'
                  ? <User className="w-3 h-3 text-[#00fff2]" />
                  : <Bot className="w-3 h-3 text-muted-foreground" />}
              </div>
              <div className={`max-w-[82%] rounded-xl px-3 py-2 text-[11px] leading-relaxed
                ${msg.role === 'user'
                  ? 'bg-[#00fff2]/20 border border-[#00fff2]/20 text-foreground'
                ...
                <div className="flex gap-1 items-center h-4">
                  {[0,0.2,0.4].map((d, i) => (
                    <motion.div key={i} animate={{ opacity: [0.3, 1, 0.3] }} transition={{ repeat: Infinity, duration: 1.2, delay: d }}
                      className="w-1.5 h-1.5 rounded-full bg-[#00fff2]/60" />
                  ))}
                </div>
              </div>
            </motion.div>
        ...
        <div className="px-4 py-3 border-t border-white/10">
          <div className="flex gap-2 items-end">
            <textarea
              ...
              rows={2}
              className="flex-1 px-3 py-2 text-[11px] bg-white/5 border border-white/10 rounded-xl focus:outline-none focus:ring-1 focus:ring-[#00fff2]/50 focus:border-[#00fff2]/30 transition-all resize-none placeholder:text-muted-foreground/50 disabled:opacity-50"
            />
            <motion.button
              onClick={handleSend}
              disabled={!input.trim() || loading}
              whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
              className="flex-shrink-0 w-9 h-9 rounded-xl bg-[#00fff2] flex items-center justify-center disabled:opacity-40 transition-all"
            >
