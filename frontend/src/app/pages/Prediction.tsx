...
export function Prediction() {
    ...
    <div className="space-y-3">
      ...
      {!convStarted ? (
        <motion.button onClick={handleStartConversation} disabled={convLoading}
          className="w-full px-3 py-2 text-xs rounded-lg bg-[#c0c0c0] text-black font-medium flex items-center justify-center gap-1.5 disabled:opacity-50"
        ...
        <div className="flex flex-col gap-2">
          <div className="text-[10px] text-muted-foreground px-1">
            <span className="text-[#c0c0c0]">{Object.keys(convParams).length}</span> parameters collected
        ...
        <div className="space-y-3">
          <div className="space-y-1 text-[10px] max-h-32 overflow-y-auto pr-1">
            ...
            {Object.entries(hybridExtracted).map(([k, v]) => (
              <div key={k} className="flex justify-between"><span className="text-muted-foreground">{k.replace(/_/g,' ')}</span><span className="text-[#c0c0c0]">{String(v)}</span></div>
            ))}
          </div>
  ...
  const renderDocumentInput = () => (
    <div className="space-y-3 relative">
      ...
      {docResult && <div className="text-[10px] text-[#c0c0c0]">● Document analysed — see results</div>}
    </div>
  );
  ...
  const renderComparativeInput = () => (
    <div className="space-y-3">
      ...
      }} className="w-full px-2 py-1.5 text-[10px] rounded-lg bg-[#c0c0c0]/10 border border-primary/20 text-[#c0c0c0] hover:bg-[#c0c0c0]/20 transition-all flex items-center justify-center gap-1">
      ...
      {!monSession ? (
        ...
        <>
          <div className="text-[10px] space-y-1">
            {[['Session', monSession.name, 'text-[#c0c0c0]'], ['Baseline', `${monSession.baseline}%`, 'font-medium'], ['Updates', String(monSession.updates.length), '']].map(([l, v, c]) => (
  ...
  const renderAdversarialInput = () => {
    ...
    return (
      <div className="space-y-3">
        <div className="p-2 rounded-lg bg-[#ff6b6b]/10 border border-destructive/20">
          <p className="text-[10px] text-[#ff6b6b]/80 leading-relaxed">
        ...
        <div className="space-y-0.5 text-[10px]">
          ...
          {Object.entries(preset).map(([k, v]) => (
            <div key={k} className="flex justify-between"><span className="text-muted-foreground">{k.replace(/_/g,' ')}</span><span className="text-[#ff6b6b] font-medium">{String(v)}</span></div>
          ))}
        </div>
    ...
    <div className="space-y-3">
      ...
      {paramList.length > 0 && (
        <motion.button onClick={() => { setModalStep(0); setModalOpen(true); }}
          ...
          <Users className="w-3 h-3 text-[#c0c0c0]" />
        </motion.button>
      )}
  ...
  const renderSimulationInput = () => (
    <div className="space-y-3">
      <div className="p-2 rounded-lg bg-[#c0c0c0]/5 border border-primary/20">
        <p className="text-[10px] text-[#c0c0c0]/80"><FlaskConical className="w-3 h-3 inline mr-1" />Hypothetical scenario — Monte Carlo simulation with 200 runs.</p>
      </div>
      <textarea className="w-full h-16 px-3 py-2 text-xs bg-white/5 border border-white/10 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/50 resize-none placeholder:text-muted-foreground/50"
      ...
      {selectedMode === 'guided' && Object.keys(parameters).length > 0 && (
        <div className="text-[10px] text-muted-foreground/70 space-y-0.5 max-h-20 overflow-y-auto">
          {Object.entries(parameters).slice(0, 6).map(([k, v]) => v != null && (
            <div key={k} className="flex justify-between"><span>{k.replace(/_/g,' ')}</span><span className="text-[#c0c0c0]/80">{String(v)}</span></div>
          ))}
        </div>
      )}
    </div>
        ...
        <div className="flex-1 overflow-y-auto space-y-3 pr-1 mb-3">
          {convMessages.length === 0 && !convStarted && (
            <div className="flex flex-col items-center justify-center h-64 text-center">
              <div className="w-12 h-12 rounded-full bg-[#c0c0c0]/10 flex items-center justify-center mb-3">
                <MessageCircle className="w-6 h-6 text-[#c0c0c0]" />
              </div>
          ...
          {convMessages.map((msg, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
              ...
              <div className={`max-w-[85%] px-3 py-2 rounded-xl text-xs leading-relaxed ${msg.role === 'sambhav' ? 'bg-white/5 border border-white/10 text-foreground' : 'bg-[#c0c0c0]/20 border border-primary/30 text-[#c0c0c0]'}`}>
                {msg.role === 'sambhav' && <span className="text-[9px] text-[#c0c0c0]/60 font-medium block mb-0.5">Sambhav</span>}
                {msg.content}
              </div>
            </motion.div>
          ))}
            ...
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="flex flex-wrap gap-1.5 pl-1">
              {currentOptions.map((opt, oi) => (
                <motion.button key={oi} onClick={() => handleConvAnswer(opt, currentKey)}
                  className="px-2.5 py-1 text-[10px] rounded-full bg-white/5 border border-white/15 hover:bg-[#c0c0c0]/15 hover:border-primary/40 transition-all"
        ...
        {convStarted && !convComplete && !isGenerating && (
          <div className="flex gap-2 pt-2 border-t border-white/10">
            ...
            <motion.button onClick={() => convInput.trim() && handleConvAnswer(convInput.trim(), currentKey)} disabled={!convInput.trim()}
              className="px-2.5 py-2 rounded-lg bg-[#c0c0c0] text-black disabled:opacity-40"
  ...
  const renderAdversarialPanel = () => {
    ...
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
        <div className={`p-3 rounded-xl border ${blocked ? 'bg-[#ff6b6b]/10 border-destructive/30' : 'bg-warning/10 border-warning/30'}`}>
          <div className="flex items-center gap-2 mb-1">
            <ShieldAlert className={`w-4 h-4 ${blocked ? 'text-[#ff6b6b]' : 'text-[#c0c0c0]'}`} />
            <span className={`text-xs font-bold ${blocked ? 'text-[#ff6b6b]' : 'text-[#c0c0c0]'}`}>
              {blocked ? 'PREDICTION BLOCKED — AUDIT SYSTEM FIRED' : 'PREDICTION PASSED WITH FLAGS'}
            </span>
          </div>
          {adversarialResult.block_reason && <p className="text-[11px] text-[#ff6b6b]/80 mt-1">{adversarialResult.block_reason}</p>}
        </div>
        <div>
          ...
          {flags.length > 0 ? flags.map((f: any, i: number) => (
            <div key={i} className="flex items-start gap-2 mb-2 p-2 rounded-lg bg-white/5 border border-white/10">
              <div className={`w-1.5 h-1.5 rounded-full mt-1 shrink-0 ${f.severity === 'CRITICAL' ? 'bg-[#ff6b6b]' : f.severity === 'WARNING' ? 'bg-warning' : 'bg-[#c0c0c0]/50'}`} />
        ...
        <motion.button onClick={() => setChatOpen(true)} className="w-full mt-4 px-3 py-2 text-xs rounded-xl bg-[#c0c0c0]/10 text-[#c0c0c0] hover:bg-[#c0c0c0]/20 border border-primary/20 transition-all flex items-center justify-center gap-1.5 font-medium">
    ...
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
        <div className="p-3 rounded-xl bg-white/5 border border-white/10">
          ...
          <p className="text-2xl font-bold text-[#c0c0c0]">{basePct}%</p>
            ...
            return (
              <div key={i} className="p-3 rounded-xl bg-white/5 border border-white/10 space-y-1.5">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5"><GitBranch className="w-3 h-3 text-[#c0c0c0]/60" /><span className="text-[11px] font-medium">{b.event || b.scenario || `Branch ${i+1}`}</span></div>
                  ...
                  return (
                    <div key={j} className="ml-4 p-2 rounded-lg bg-white/5 border border-white/5 flex justify-between">
                      ...
                      <div className="flex items-center gap-1">
                        <span className={`text-[9px] font-mono ${ss >= 0 ? 'text-[#c0c0c0]' : 'text-[#ff6b6b]'}`}>{ss >= 0 ? '+' : ''}{Math.round(ss)}%</span>
        ...
        <motion.button onClick={() => setChatOpen(true)} className="w-full mt-4 px-3 py-2 text-xs rounded-xl bg-white/5 text-muted-foreground hover:text-foreground border border-white/10 hover:bg-white/10 transition-all flex items-center justify-center gap-1.5 font-medium">
          <MessageCircle className="w-4 h-4 text-[#c0c0c0]" /> Discuss these scenarios with AI
        </motion.button>
    ...
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
        {compResult.recommendation && (
          <div className="p-3 rounded-xl bg-[#c0c0c0]/10 border border-primary/30">
            <p className="text-[10px] text-[#c0c0c0]/80 font-medium">Recommendation</p>
                ...
                return (
                  <tr key={oi} className="border-t border-white/5">
                    ...
                    {vals.map((v: any, vi: number) => (
                      <td key={vi} className={`py-1.5 px-2 text-center font-mono ${vi === winI ? 'text-[#c0c0c0] font-bold' : ''}`}>{v !== null ? `${Math.round(v)}%` : '—'}</td>
                    ))}
        ...
        <motion.button onClick={() => setChatOpen(true)} className="w-full mt-4 px-3 py-2 text-xs rounded-xl bg-white/5 text-muted-foreground hover:text-foreground border border-white/10 hover:bg-white/10 transition-all flex items-center justify-center gap-1.5 font-medium">
          <MessageCircle className="w-4 h-4 text-[#c0c0c0]" /> Ask the AI to evaluate this comparison
        </motion.button>
  ...
  const renderExpertPanel = () => {
    const agentColors: Record<string, string>  = { optimist: 'text-[#c0c0c0]', pessimist: 'text-[#ff6b6b]', realist: 'text-[#c0c0c0]', devils_advocate: 'text-[#c0c0c0]' };
    ...
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
          ...
          <>
            {expertDebate.final_probability != null && (
              <div className="p-3 rounded-xl bg-white/5 border border-white/10">
                ...
                <p className="text-2xl font-bold text-[#c0c0c0]">{Math.round(expertDebate.final_probability * 100)}%</p>
              </div>
            )}
            <div className="space-y-2">
        ...
        <motion.button onClick={() => setChatOpen(true)} className="w-full mt-4 px-3 py-2 text-xs rounded-xl bg-white/5 text-muted-foreground hover:text-foreground border border-white/10 hover:bg-white/10 transition-all flex items-center justify-center gap-1.5 font-medium">
          <MessageCircle className="w-4 h-4 text-[#c0c0c0]" /> Discuss this debate with the moderator
        </motion.button>
    ...
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
        <div className="p-3 rounded-xl bg-white/5 border border-white/10">
          ...
          <p className="text-2xl font-bold text-[#c0c0c0]">{retroResult.probability_at_time ?? '?'}%</p>
        </div>
        ...
        <motion.button onClick={() => setChatOpen(true)} className="w-full mt-4 px-3 py-2 text-xs rounded-xl bg-white/5 text-muted-foreground hover:text-foreground border border-white/10 hover:bg-white/10 transition-all flex items-center justify-center gap-1.5 font-medium">
          <MessageCircle className="w-4 h-4 text-[#c0c0c0]" /> Analyse this event retrospectively with AI
        </motion.button>
    ...
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
        <div className="flex gap-3">
          {[['Baseline', `${monSession.baseline}%`, 'text-muted-foreground'], ['Current', `${pts[pts.length-1].probability}%`, 'text-[#c0c0c0]'], ['Updates', String(monSession.updates.length), '']].map(([l, v, c]) => (
            ...
            return (
              <div key={i} className="flex items-center gap-2 p-2 rounded-lg bg-white/5 border border-white/10">
                ...
                <div className="text-right">
                  ...
                  {i > 0 && <p className={`text-[9px] font-mono ${delta >= 0 ? 'text-[#c0c0c0]' : 'text-[#ff6b6b]'}`}>{delta >= 0 ? '+' : ''}{delta.toFixed(1)}%</p>}
                </div>
              </div>
            );
          })}
          ...
          <div className="grid grid-cols-2 gap-2">
            {[
              ...
              <div key={l} className="p-2 rounded-xl bg-white/5 border border-white/10 text-center">
                <p className="text-[9px] text-muted-foreground">{l}</p><p className="text-sm font-bold text-[#c0c0c0]">{v}</p>
              </div>
            ))}
          </div>
        )}
  ...
  const renderStandardResults = () => (
    <motion.div key="results" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-4">
      <div className="flex items-start justify-between mb-3">
        ...
        <button onClick={() => setChatOpen(true)} className="px-3 py-1.5 text-[10px] rounded-lg bg-[#c0c0c0]/20 text-[#c0c0c0] border border-primary/30 hover:bg-[#c0c0c0]/30 transition-all flex items-center gap-1.5 font-medium">
          ...
          <div key={idx}>
                  ...
                  {loadingWhy === idx ? (
                    ...
                    <div className="space-y-3">
                      ...
                      {activeWhyData.detailed && (
                        <div className="grid grid-cols-2 gap-2">
                          <div className="p-2 rounded-lg bg-[#c0c0c0]/5 border border-primary/15">
                            <p className="text-[9px] text-[#c0c0c0] font-bold mb-1 uppercase tracking-wide">Case For ({outcome.probability.toFixed(0)}%)</p>
                            <p className="text-[10px] text-muted-foreground leading-relaxed">{activeWhyData.detailed.case_for}</p>
                          </div>
                          <div className="p-2 rounded-lg bg-[#c0c0c0]/5 border border-secondary/15">
                            <p className="text-[9px] text-[#c0c0c0] font-bold mb-1 uppercase tracking-wide">Case Against</p>
                      ...
                      {activeWhyData.full && (
                        <div className="space-y-1.5 pt-2 border-t border-white/10">
                          {activeWhyData.full.primary_driver && <p className="text-[10px] text-muted-foreground"><span className="text-[#c0c0c0] font-semibold">Primary Driver:</span> {activeWhyData.full.primary_driver}</p>}
                          {activeWhyData.full.intervention && <p className="text-[10px] text-muted-foreground"><span className="text-[#c0c0c0] font-semibold">Intervention:</span> {activeWhyData.full.intervention}</p>}
                          {(activeWhyData.full.confidence_note || activeWhyData.full.confidence_factors) && <p className="text-[10px] text-muted-foreground"><span className="text-[#c0c0c0] font-semibold">Confidence:</span> {activeWhyData.full.confidence_note || activeWhyData.full.confidence_factors}</p>}
                        </div>
                      )}
                    </div>
                  ) : (
            ...
            <div className="flex items-center gap-1.5 mt-1.5 ml-0.5">
              <motion.button
                ...
                className="px-2 py-0.5 text-[9px] rounded border border-destructive/25 bg-[#ff6b6b]/5 text-[#ff6b6b]/60 hover:bg-[#ff6b6b]/10 hover:text-[#ff6b6b]/80 hover:border-destructive/35 transition-all flex items-center gap-1 disabled:opacity-40"
            ...
            <AnimatePresence>
              {inverseData[`inv_${idx}`] && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
                  className="mt-2 p-3 rounded-lg bg-[#ff6b6b]/5 border border-destructive/20 space-y-2">
                  <div className="flex items-center gap-1.5">
                    <AlertCircle className="w-3 h-3 text-[#ff6b6b]/60 shrink-0" />
                    <span className="text-[10px] font-semibold text-[#ff6b6b]/80">
                  ...
                  {inverseData[`inv_${idx}`].inverse_scenario?.trigger_factors?.length > 0 && (
                    <div>
                      <p className="text-[9px] text-[#ff6b6b]/60 font-semibold mb-0.5 uppercase tracking-wide">Trigger Factors</p>
                  ...
                  {inverseData[`inv_${idx}`].inverse_scenario?.early_warnings?.length > 0 && (
                    <div>
                      <p className="text-[9px] text-[#c0c0c0]/70 font-semibold mb-0.5 uppercase tracking-wide">Early Warning Signs</p>
          ...
          <DialogTrigger asChild>
            <button className="w-full relative overflow-hidden group p-4 rounded-xl bg-gradient-to-br from-white/5 to-white/5 border border-white/20 hover:border-primary/50 transition-all text-left">
              ...
              <div className="flex items-center justify-between relative z-10">
                <div>
                  <p className="text-[11px] text-[#c0c0c0] font-semibold uppercase tracking-widest mb-1 flex items-center gap-2">
                ...
                <div className="text-[#c0c0c0] group-hover:translate-x-1 transition-transform">
          ...
          <DialogContent className="max-w-2xl bg-[#0a0a0f] border-white/10 p-0 text-white overflow-hidden max-h-[85vh] flex flex-col">
            <DialogHeader className="p-5 border-b border-white/10 shrink-0">
              <DialogTitle className="text-lg font-bold flex items-center gap-2">
                <ShieldAlert className="w-5 h-5 text-[#c0c0c0]" /> PRAGMA Complete Analysis
              </DialogTitle>
            ...
            <div className="p-5 overflow-y-auto space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 rounded-lg bg-white/5 border border-white/10">
                  <p className="text-[10px] text-[#c0c0c0] font-semibold uppercase tracking-wider mb-2">Primary Motive / Trigger</p>
                ...
                <div className="p-4 rounded-lg bg-white/5 border border-white/10">
                  <p className="text-[10px] text-[#c0c0c0] font-semibold uppercase tracking-wider mb-2">Psychological State</p>
                  <p className="text-xs leading-relaxed text-muted-foreground">
                    {isDeceptive
                ...
                <div className="p-4 rounded-lg bg-white/5 border border-white/10 col-span-2">
                  <p className="text-[10px] text-[#c0c0c0] font-semibold uppercase tracking-wider mb-2">Linguistic Breakdown</p>
              ...
              <div>
                <p className="text-[10px] text-[#c0c0c0] font-semibold uppercase tracking-wider mb-3">Consult the Profiler</p>
                <PragmaChat 
                   predictionId={predId} 
  ...
  const renderRightPanel = () => {
    if (selectedMode === 'conversational') {
      return showResults ? (
        <div className="space-y-4">
          <div className="p-3 rounded-xl bg-[#c0c0c0]/10 border border-primary/20 mb-2">
            <p className="text-[10px] text-[#c0c0c0]/80">● Conversation complete — prediction from {Object.keys(convParams).length} parameters</p>
          </div>
          {renderStandardResults()}
        </div>
    ...
    return (
      <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="h-full flex flex-col items-center justify-center text-center">
        <div className="w-16 h-16 rounded-full bg-[#c0c0c0]/10 flex items-center justify-center mb-3"><Play className="w-8 h-8 text-[#c0c0c0]" /></div>
        ...
        <div className="max-w-7xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-4">
            ...
            <p className="text-[11px] text-muted-foreground">
              Mode: <span className="text-[#c0c0c0]">{selectedMode}</span>
              {selectedMode !== 'free' && selectedMode !== 'adversarial' && <> · Domain: <span className="text-[#c0c0c0]">{selectedDomain.replace(/_/g,' ')}</span></>}
            </p>
          </motion.div>
          {domainInfo?.disclaimer && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-3 px-3 py-2 rounded-lg bg-[#c0c0c0]/5 border border-[#c0c0c0]/20 flex items-start gap-2">
              <AlertCircle className="w-3 h-3 text-[#c0c0c0] mt-0.5 shrink-0" />
              <p className="text-[10px] text-[#c0c0c0]/80">{domainInfo.disclaimer}</p>
            </motion.div>
          )}
              ...
              <GlassCard variant="elevated" className="p-4 relative overflow-hidden">
                ...
                {isAnalyzing && (
                  <div className="absolute inset-0 z-50 bg-background/80 backdrop-blur-sm flex flex-col items-center justify-center p-6 text-center">
                    ...
                    <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="mt-4 text-xs font-medium text-[#c0c0c0]">
                ...
                {!hideGenBtn && selectedMode !== 'conversational' && (
                  <div className="mt-3 flex gap-2">
                    <motion.button onClick={() => handleGenerate()} disabled={isGenerating}
                      className="flex-1 px-3 py-2 text-xs rounded-lg bg-[#c0c0c0] text-black font-medium flex items-center justify-center gap-1.5 disabled:opacity-50"
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mt-3">
                  <GlassCard variant="elevated" className="p-3">
                    ...
                    <div className="space-y-1.5 text-[10px]">
                      {[
                        ['ML Probability', predResult.ml_probability != null ? `${(predResult.ml_probability * 100).toFixed(1)}%` : '—', 'text-[#c0c0c0]'],
                        ...
                        ['Reliability', `${(predResult.reliability_index * 100).toFixed(0)}%`, 'text-[#c0c0c0]'],
                      ].map(([l, v, c]) => (
                        <div key={l as string} className="flex justify-between"><span className="text-muted-foreground">{l as string}</span><span className={c as string}>{v as string}</span></div>
                      ))}
                    </div>
