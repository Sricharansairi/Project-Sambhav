...
def llm_predict(domain: str, parameters: dict, question: str = None) -> dict:
    ...
    system_msg = {
        ...
        "content": (
            ...
            "- student (exam/placement success): 55%\n"
            "- high_school (pass/promotion): 70%\n"
            "- job_life (promotion/retention): 52%\n"
            "- health (disease risk): 40%\n"
            "- fitness (obesity/fitness risk): 45%\n"
            "- financial (investment success/default risk): 38%\n"
            "- mental_health (burnout/wellness risk): 42%\n"
            "- behavioral (habit/addiction risk): 45%\n"
            "- claim (claim validity): 50%\n"
            "- pragma (startup/project success): 35%\n"
            "- sarvagna (general prediction): 50%\n"
            "\nCALIBRATION RULES — STRICTLY FOLLOW:\n"
            "1. Start from the domain base rate above\n"
            "2. Each STRONG positive signal → shift UP by +8 to +18%\n"
            "3. Each STRONG negative signal → shift DOWN by -8 to -18%\n"
            "4. Each WEAK signal → shift by ±3 to 5%\n"
            "5. 2+ strong positive signals → result MUST be above 65%\n"
            "6. 2+ strong negative signals → result MUST be below 35%\n"
            "7. Mixed signals → stay within 35-65% range\n"
            "8. NEVER output exactly 50.0 — that signals no analysis\n"
            "9. NEVER be overly conservative — span the full 0-100 range\n"
