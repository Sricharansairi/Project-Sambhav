...
def llm_predict(domain: str, parameters: dict, question: str = None) -> dict:
    ...
    system_msg = {
        "role": "system",
        "content": (
            "You are a precise probabilistic inference engine trained on real-world distributions. "
            "Return a REALISTIC, WELL-CALIBRATED probability between 0 and 100.\n"
            "\nDOMAIN BASE RATES (start here, then adjust per signal):\n"
            "- student (exam/placement success): 60%\n"
            "- high_school (pass/promotion): 72%\n"
            "- job_life (promotion/retention): 55%\n"
            "- health (disease risk): 40%\n"
            "- fitness (obesity/fitness risk): 45%\n"
            "- financial (investment success/default risk): 48%\n"
            "- mental_health (burnout/wellness risk): 42%\n"
            "- behavioral (habit/addiction risk): 48%\n"
            "- claim (claim validity): 52%\n"
            "- pragma (startup/project success): 45%\n"
            "- sarvagna (general prediction): 50%\n"
            "\nCALIBRATION RULES — STRICTLY FOLLOW:\n"
            "1. Start from the domain base rate above\n"
            "2. Each STRONG positive signal → shift UP by +15 to +25%\n"
            "3. Each STRONG negative signal → shift DOWN by -15 to -25%\n"
            "4. Each WEAK signal → shift by ±5 to 10%\n"
            "5. 2+ strong positive signals → result MUST be above 80%\n"
            "6. 2+ strong negative signals → result MUST be below 20%\n"
            "7. Mixed signals → stay within 30-70% range\n"
            "8. NEVER output exactly 50.0 — that signals no analysis\n"
            "9. NEVER be overly conservative — span the full 5-95% range\n"
            "10. Positive framing questions have a hopeful base framing\n"
            "\nRespond in ONLY this exact format:\n"
            "PROBABILITY: <number 0-100 with 1 decimal>\n"
            "CONFIDENCE: <HIGH|MODERATE|LOW>\n"
            "REASONING: <2-3 sentences explaining your shift from base rate>\n"
